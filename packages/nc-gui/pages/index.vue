<!--<template>
  <div id="app">
   <iframe :src="source" style=  "height: 100vh;
  width: 100vw;
  border: none;
...
var slide = require('./static/js/main.js');
...
export default {
  ...
  layout: "empty",
  data: () => ({
    source: "https://buildboard-nocodb-home.herokuapp.com",
    carItems: [
    ...
    defaultMessage:
      "Looks like you configured databases.<br> Now it's time to setup an admin user.",
    loading: true,
  ...
  computed: {
    text() {
      const text = this.lang.find(
        (it) => it.symbol === this.$store.state.windows.language
      );
  ...
  // metaInfo: {
  //   script: [
  //     { src: 'http://localhost:3000/_nuxt/pages/static/js/main.js', async: true, defer: true }
  //   ],
  // },
  created() {
    ...
    if (projectInfo) {
      if (
        ...
        (projectInfo && projectInfo.authType === "none")
      ) {
